...
export default function PWAInstallButton({ theme = "auto" }) {
	...
	// 🧠 Detect install prompt + installed state
	useEffect(() => {
	...
	const handleInstall = () => {
		if (installPrompt) {
			...
			// fallback if no native install prompt
			alert("To install the app, open your browser’s share menu and choose 'Add to Home Screen'.");
		}
	};

	// 🧩 Hide only if already installed or in standalone mode
